...
//
    ...
    // Backbone Controllers are almost more convention than they are actual code. A Controller
    // is simply a JavaScript object that represents a logical chunk of UI in the
    // DOM. This might be a single item, an entire list, a sidebar or panel, or
    // even the surrounding frame which wraps your whole app. Defining a chunk of
    // UI as a **Controller** allows you to define your DOM events declaratively, without
    // having to worry about render order ... and makes it easy for the controller to
    // react to specific changes in the state of your models.

    // Creating a Backbone.Controller creates its initial element outside of the DOM,
    // if an existing element is not provided...
    ...
    var controllerOptions = ['model', 'collection', 'el', 'id', 'attributes', 'className', 'tagName', 'events'];

    // Set up all inheritable **Backbone.Controller** properties and methods.
    _.extend(Controller.prototype, Events, {

        // The default `tagName` of a Controller's element is `"div"`.
        tagName: 'div',

        // jQuery delegate for element lookup, scoped to DOM elements within the
        // current controller. This should be prefered to global lookups where possible.
        $: function(selector) {
            return this.$el.find(selector);
        },
        ...
        // **render** is the core function that your controller should override, in order
        // to populate its element (`this.el`), with the appropriate HTML. The
        // convention is for **render** to always return `this`.
        render: function() {
            return this;
        ...
        // Remove this controller by taking the element out of the DOM, and removing any
        // applicable Backbone.Events listeners.
        remove: function() {
            this.$el.remove();
            this.stopListening();
        ...
        // Change the controller's element (`this.el` property), including event
        // re-delegation.
        setElement: function(element, delegate) {
            if (this.$el) this.undelegateEvents();
            this.$el = element instanceof Backbone.$ ? element : Backbone.$(element);
        ...
        // }
        //
        // pairs. Callbacks will be bound to the controller, with `this` set properly.
        // Uses event delegation for efficiency.
        // Omitting the selector binds the event to `this.el`.
        // This only works for delegate-able events: not `focus`, `blur`, and
        // not `change`, `submit`, and `reset` in Internet Explorer.
        ...
        // Clears all callbacks previously bound to the controller with `delegateEvents`.
        // You usually don't need to use this, but may wish to if you have multiple
        // Backbone controllers attached to the same DOM element.
        undelegateEvents: function() {
            this.$el.off('.delegateEvents' + this.cid);
            return this;
        },
        ...
        // Keys with special meaning *(e.g. model, collection, id, className)* are
        // attached directly to the controller. See `controllerOptions` for an exhaustive
        // list.
        _configure: function(options) {
            if (this.options) options = _.extend({}, _.result(this, 'options'), options);
        ...
        // Ensure that the Controller has a DOM element to render into.
        // If `this.el` is a string, pass it through `$()`, take the first
        // matching element, and re-assign it to `el`. Otherwise, create
        // an element from the `id`, `className` and `tagName` properties.
        _ensureElement: function() {
...
//
    ...
    Model.extend = Controller.extend = Collection.extend = Router.extend = View.extend = History.extend = extend;
